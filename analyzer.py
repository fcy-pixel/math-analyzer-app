  ...
  "parent_teacher_report": {{
    ...
    def analyze_student_paper(
        ...
        system_msg = (
            ...
            "重要評分規則：香港小學數學課程接受假分數（improper fraction）作為正確答案，"
            "例如 7/4 與 1 3/4 均視為正確，不應因為學生以假分數作答而扣分或標記為錯誤。"
        )
...
5. **假分數視為正確**：若答案鍵為帶分數（如 1 3/4），學生以等值假分數（如 7/4）作答，應視為正確，反之亦然。只要數值相等即可得分
...
5. **假分數視為正確**：假分數（如 7/4）與帶分數（如 1 3/4）均為可接受的正確答案形式，只要數值相等即可
